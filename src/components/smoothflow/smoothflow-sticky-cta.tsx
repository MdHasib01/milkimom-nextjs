...
export function SmoothflowStickyCTA() {
  ...
  return (
    <div
      className={cn(
        "fixed inset-x-0 bottom-0 z-50 w-full max-w-full overflow-hidden border-t border-brand/20 bg-white/95 px-4 py-2.5 backdrop-blur-md transition-all duration-300 shadow-[0_-8px_25px_rgba(0,0,0,0.1)]",
        scrolled
      ...
      <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-3 min-w-0">
        ...
        <div className="flex flex-col gap-0.5 min-w-0 flex-1 overflow-hidden">
          <div className="flex items-center gap-2 min-w-0 flex-wrap">
            <span className="font-extrabold text-base sm:text-xl text-brand leading-none whitespace-nowrap">
              ৳১,৯৯৯
            </span>
            <span className="text-xs sm:text-sm text-[#1A1A1A]/40 line-through decoration-1 leading-none whitespace-nowrap">
              ৳৩,৪৫০
            </span>
            <span className="rounded-md bg-brand-crimson/10 px-1.5 py-0.5 text-[10px] sm:text-xs font-bold text-brand-crimson leading-none whitespace-nowrap">
              ৪২% ছাড়
            </span>
          </div>

          {countdown && (
            <div className="inline-flex items-center gap-1 w-fit rounded border border-brand-crimson/20 bg-brand-crimson/5 px-1.5 py-0.5 text-[10px] sm:text-xs font-medium text-brand-crimson min-w-0 mt-0.5">
